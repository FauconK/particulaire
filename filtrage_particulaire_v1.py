...
#im3,kmeans2,histo2=calcul_histogramme(new_im,selectionner_zone(),Nb)

def calcul_de_vraissemblance(histo, histo_ref):
    return np.exp(-lambda_im(1-((histo*histo_ref).sum())**(1/2)))
    
    
    
def filtrage_particulaire():
    #initialisation des particules
    X_0 = np.random.randn(Nb,2)*17.4
    w = norm.pdf(np.array(zoneAT[0],zoneAT[1]),X_0,pow(300,0.5))
    w= w/w.sum()
    ...
    for t in range(1,T):
        ...
        reech = x[t-1,:][A]
        x[t,:] =  0.5*reech + 25*(reech/(1+reech**2)) + 8*np.cos(np.ones(N)*t*1.2) + np.random.normal(0, Q, N)
        w = np.zeros(T)
        w = norm.pdf(vecteur_y[t],x[t,:]**2/20,R)
        w=w/w.sum()
...
print('tatatata')
#truc de pixels: une faible variation de la couleur rgb fait que c'est plus du tout reconnu on pourrait modifier en mettant un distance linéaire ou quadratique plutot que "tout ou rien"

#a,b,histo = calcul_histogramme(imt,zonei,Nb)